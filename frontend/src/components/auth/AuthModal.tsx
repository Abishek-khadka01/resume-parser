        ...
        <div className="w-full max-w-[440px] bg-white rounded-2xl shadow-2xl overflow-hidden pointer-events-auto animate-in fade-in zoom-in-95 duration-200">
          ...
          <div className="px-8 pt-8 pb-6">
            ...
            <div className="flex mb-6 bg-slate-100 rounded-lg p-1">
              <button
                type="button"
                onClick={() => { setMode('signin'); loginForm.reset(); registerForm.reset() }}
              ...
              <button
                type="button"
                onClick={() => { setMode('signup'); loginForm.reset(); registerForm.reset() }}
                className={`flex-1 py-2 text-sm font-semibold rounded-md transition-all cursor-pointer ${
                  mode === 'signup'
                    ? 'bg-white text-[#4a0080] shadow-sm'
                    : 'text-slate-500 hover:text-slate-700'
            ...
            {mode === 'signin' ? (
              <form onSubmit={loginForm.handleSubmit(handleLogin)} className="flex flex-col gap-4">
                <div>
                  <div className="flex items-center gap-3 px-4 py-3 rounded-lg border border-slate-300 bg-white focus-within:border-[#74007a] focus-within:ring-1 focus-within:ring-[#74007a]/20 transition-all">
                    <FontAwesomeIcon icon={faEnvelope} className="text-slate-400 text-sm w-4" />
                    <input
                      type="email"
                  ...
                  {loginForm.formState.errors.email && (
                    <p className="text-xs text-red-500 mt-1 ml-1">{loginForm.formState.errors.email.message}</p>
                  )}
                </div>

                <div>
                  <div className="flex items-center gap-3 px-4 py-3 rounded-lg border border-slate-300 bg-white focus-within:border-[#74007a] focus-within:ring-1 focus-within:ring-[#74007a]/20 transition-all">
                    <FontAwesomeIcon icon={faLock} className="text-slate-400 text-sm w-4" />
                    <input
                  ...
                  {loginForm.formState.errors.password && (
                    <p className="text-xs text-red-500 mt-1 ml-1">{loginForm.formState.errors.password.message}</p>
                  )}
                </div>
              ...
              <form onSubmit={registerForm.handleSubmit(handleRegister)} className="flex flex-col gap-4">
                <div>
                  <div className="flex items-center gap-3 px-4 py-3 rounded-lg border border-slate-300 bg-white focus-within:border-[#74007a] focus-within:ring-1 focus-within:ring-[#74007a]/20 transition-all">
                    <FontAwesomeIcon icon={faUser} className="text-slate-400 text-sm w-4" />
                    <input
                      type="text"
                  ...
                  {registerForm.formState.errors.fullName && (
                    <p className="text-xs text-red-500 mt-1 ml-1">{registerForm.formState.errors.fullName.message}</p>
                  )}
                </div>
                ...
                <div>
                  ...
                  {registerForm.formState.errors.email && (
                    <p className="text-xs text-red-500 mt-1 ml-1">{registerForm.formState.errors.email.message}</p>
                  )}
                </div>
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div>
                    ...
                    {registerForm.formState.errors.password && (
                      <p className="text-xs text-red-500 mt-1 ml-1">{registerForm.formState.errors.password.message}</p>
                    )}
                  </div>
                  ...
                  <div>
                    ...
                    {registerForm.formState.errors.confirm && (
                      <p className="text-xs text-red-500 mt-1 ml-1">{registerForm.formState.errors.confirm.message}</p>
                    )}
                  </div>
                </div>
            ...
            <p className="text-xs text-slate-400 text-center mt-4">
              {mode === 'signin' ? (
                ...
                <>
                  ...
                  <button
                    type="button"
                    onClick={() => { setMode('signin'); registerForm.reset() }}
                    className="text-[#74007a] font-semibold hover:text-[#da70dc] transition-colors cursor-pointer"
                  >
                    Sign in
                  </button>
